...
CREATE INDEX IF NOT EXISTS idx_bounty_tasks_enterprise_time
    ON bounty_tasks(enterprise_user_id, created_at);

CREATE INDEX IF NOT EXISTS idx_bounty_tasks_status_time
    ON bounty_tasks(status, created_at);
...
CREATE INDEX IF NOT EXISTS idx_bounty_submissions_task_time
    ON bounty_submissions(task_id, created_at);

CREATE INDEX IF NOT EXISTS idx_bounty_submissions_student_time
    ON bounty_submissions(student_user_id, created_at);

CREATE INDEX IF NOT EXISTS idx_bounty_submissions_status_time
    ON bounty_submissions(status, created_at);

ALTER TABLE bounty_tasks
    ADD CONSTRAINT fk_bounty_tasks_accepted_submission
        FOREIGN KEY (accepted_submission_id) REFERENCES bounty_submissions(id);
