...
import { Camera, RefreshCw, Scissors, UploadCloud } from "lucide-react";
...
export type { StudentAvatarMeta } from "../../lib/studentAvatar";

export type StudentAvatarUploadResult = {
  uploaded: boolean;
  contentType: string;
  sizeBytes: number;
...
export default function StudentAvatarEditor({
  ...
  tier?: string | null;
  onAvatarUploaded: (payload: StudentAvatarUploadResult) => void;
  onToast: (message: string, tone?: ToastTone) => void;
}) {
  const fileInputRef = useRef<HTMLInputElement | null>(null);
  const dragStateRef = useRef<DragState | null>(null);
  const [avatarUrl, setAvatarUrl] = useState<string | null>(() => getCachedStudentAvatarDataUrl({ userId, avatar }));
  ...
  useEffect(() => {
    ...
    const cachedValue = getCachedStudentAvatarDataUrl({ userId, avatar });
    if (cachedValue) {
    ...
    void (async () => {
      try {
        const dataUrl = await loadStudentAvatarDataUrl({ userId, avatar });
        if (!active) {
          return;
        }
        ...
        if (active) {
          onToast(buildErrorMessage(error, "头像加载失败，请稍后重试。"), "error");
        }
      } finally {
        if (active) {
  ...
  }, [avatar?.uploaded, avatar?.updatedAt, onToast, userId]);
  ...
  const handleAvatarUpload = async () => {
    ...
    try {
      ...
      const response = await apiRequest<StudentAvatarUploadResult>("/profiles/students/me/avatar", {
        method: "POST",
        body: formData,
      });
      const nextAvatarUrl = await primeStudentAvatarCache({
        userId,
      ...
      setEditor(createAvatarEditorState());
      onToast(avatar?.uploaded ? "头像已经更新。" : "头像已经上传。");
    } catch (error) {
  ...
  const avatarFrameClassName = isPremium
    ? "from-[#fff4d6] via-[#fff9ee] to-white ring-4 ring-[#f5d487] shadow-[0_20px_50px_rgba(245,180,58,0.24)]"
    : "from-indigo-100 via-violet-50 to-white ring-4 ring-indigo-100 shadow-[0_20px_50px_rgba(99,102,241,0.18)]";
          ...
          >
            ...
            >
              <motion.div
                ...
                className="relative w-full max-w-[66rem] overflow-hidden rounded-[2.4rem] border border-white/80 bg-[linear-gradient(145deg,rgba(255,255,255,0.97),rgba(248,250,252,0.96)_38%,rgba(238,244,255,0.96))] p-5 text-slate-900 shadow-[0_32px_90px_rgba(148,163,184,0.28)] lg:p-6"
              >
                <div className="pointer-events-none absolute inset-x-0 top-0 h-40 bg-[radial-gradient(circle_at_top,rgba(99,102,241,0.18),transparent_62%)]" />
                <div className="pointer-events-none absolute -left-12 bottom-0 h-48 w-48 rounded-full bg-sky-300/18 blur-3xl" />
                ...
                <div className="relative z-10">
                  <div className="flex flex-col gap-4 border-b border-slate-200/80 pb-5 lg:flex-row lg:items-end lg:justify-between">
                    <div>
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <h3 className="mt-1 text-3xl font-semibold text-slate-900 lg:text-[2.1rem]">调整头像</h3>
                        </div>
                      </div>
                      <p className="mt-4 max-w-2xl text-base leading-8 text-slate-600">
                        调整到满意的位置后直接保存，新的头像会立即同步到资料页展示。
                      </p>
                    </div>
                    <div className="flex flex-wrap items-center gap-3">
                      ...
                      <button
                        ...
                        disabled={editor.busy}
                        className="inline-flex items-center justify-center rounded-full border border-indigo-200 bg-indigo-50 px-4 py-2.5 text-base font-semibold text-indigo-700 transition-colors hover:border-indigo-300 hover:bg-indigo-100 disabled:cursor-not-allowed disabled:opacity-60"
                      >
                  ...
                  <div className="mt-6 grid gap-5 xl:justify-center xl:grid-cols-[40rem_20rem]">
                    <div
                      className="relative h-[440px] w-[640px] cursor-grab overflow-hidden rounded-[2rem] border border-slate-200/90 bg-[radial-gradient(circle_at_top,rgba(99,102,241,0.12),transparent_58%),linear-gradient(180deg,#fdfefe_0%,#edf4ff_100%)] shadow-[0_24px_60px_rgba(148,163,184,0.22)] active:cursor-grabbing"
                    ...
                    >
                      ...
                      />
                      <div
                        className="pointer-events-none absolute rounded-[1.7rem] border border-indigo-300 shadow-[0_0_0_1px_rgba(165,180,252,0.3)]"
                        style={{
                          left: cropFrameInsetX,
                          top: cropFrameInsetY,
                          width: CROP_VIEWPORT_SIZE,
                      ...
                      />
                      <div
                        className="pointer-events-none absolute w-px bg-indigo-200/80"
                        style={{
                      ...
                      />
                      <div
                        className="pointer-events-none absolute h-px bg-indigo-200/80"
                        style={{
                          left: cropFrameInsetX,
                    ...
                    <div className="flex h-[440px] w-80 flex-col">
                      <div className="flex h-full flex-col rounded-[1.8rem] border border-slate-200/80 bg-white/88 p-5 shadow-[0_18px_45px_rgba(148,163,184,0.14)]">
                        <div className="text-base font-semibold text-slate-900">头像预览</div>
                        <div className="mt-4 flex items-center gap-4">
                          <div className={`relative flex h-28 w-28 items-center justify-center overflow-hidden rounded-[2rem] bg-gradient-to-br ${avatarFrameClassName} text-3xl font-black text-slate-900`}>
                            <img
                              src={editor.sourceDataUrl}
                              alt="头像效果预览"
                              draggable={false}
                        ...
                        <div className="mt-auto pt-4">
                          <div className="flex flex-col gap-3">
                            ...
                            <button
                              ...
                              className="inline-flex w-full items-center justify-center rounded-2xl bg-indigo-600 px-6 py-3.5 text-base font-semibold text-white shadow-[0_18px_32px_rgba(79,70,229,0.24)] transition-transform hover:-translate-y-0.5 hover:bg-indigo-500 disabled:cursor-not-allowed disabled:opacity-70"
                            >
                              {editor.busy ? (
                                <>
    ...
    <>
      <div className="flex w-28 flex-col items-center text-center">
        <button
          type="button"
          onClick={openFilePicker}
          className={`group relative flex h-28 w-28 items-center justify-center overflow-hidden rounded-[1.85rem] bg-gradient-to-br ${avatarFrameClassName} text-[1.95rem] font-black text-slate-900 transition-transform hover:-translate-y-1`}
        >
          {avatarUrl ? (
            <img src={avatarUrl} alt={`${heroName} 的头像`} className="h-full w-full object-cover" />
          ) : (
          ...
          <div className="absolute inset-x-0 bottom-0 flex items-center justify-center gap-2 bg-slate-950/72 px-3 py-2 text-xs font-semibold text-white opacity-0 transition-opacity group-hover:opacity-100">
            {avatarLoading ? <RefreshCw size={14} className="animate-spin" /> : <Camera size={14} />}
            {avatar?.uploaded ? "修改头像" : "上传头像"}
          </div>
